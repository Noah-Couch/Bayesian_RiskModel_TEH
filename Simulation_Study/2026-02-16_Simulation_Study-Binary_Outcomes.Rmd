---
title: "Binary Trial Outcomes -- Simulation"
author: "Noah Couch"
date: "2026-02-16"
...
"model{  
  ...
  for(i in 1:n){
  Y[i]  ~  dbern(mu[i])
  logit(mu[i]) <- beta0 + inprod(beta[],X[i,])
  }
  #Priors
  for(j in 1:p){
  gamma[j] ~ dnorm(0,0.1)
...
Outcomes <- data.frame(rejection.Risk = rep(NA, N),
                       ...
                       SSVS_Converge = rep(NA,N),
                       Risk_Converge = rep(NA,N),
                       Htrg_Converge = rep(NA,N),
...
for (i in 1:N) {
    ...
    Simulated_Data <- tibble(PID, Treatment, Gender, Age, ProgVar)
    ...
    beta.GeneratingOutcome <- case_when(
      ...
      ProgVar.type == "Categorical" ~  c(-0.25,    # Intercept
                                         0.50,    # Treatment
                                         0.25,    # Categorical 
                                         0.50,    # Treatment * Categorical Predictor
                                         -0.50,   # Gender
                                         -0.50),  # Age
      ...
      ProgVar.type == "Continuous" ~   c(-0.25,    # Intercept
                                         0.50,    # Treatment
                                         0.25,    # Continuous 
                                         0.50,    # Treatment * Continuous Predictor
                                         -0.50,   # Gender
                                         -0.50),  # Age
      TRUE ~ NA)
    ...
    Trial_Data <- Simulated_Data |>
      rowwise() |>
      mutate(logit_mu = beta.GeneratingOutcome[1] + 
             ...
             Outcome = rbinom(n = 1, size = 1, prob = mu )) |> 
      select(-c(mu, logit_mu))
    ...
    Converge.SVSS <- if_else(all(gelman.diag(JAGS_samps.SSVS)$psrf[,2]) < 1.1,
    ...
    Converge.Htrg <- if_else(all(gelman.diag(JAGS_samps.Heterogeneity)$psrf[,2]) < 1.1,
                             "Converged", "Not Converged")
    ...
    mod.Age <- glm(Outcome ~ Treatment * Age + Gender + ProgVar + Outcome_BL,
                  data = Trial_Data, family = "binomial")
    
    mod.Gen <- glm(Outcome ~ Treatment * Gender + Age + ProgVar + Outcome_BL,
                  data = Trial_Data, family = "binomial")
    
    mod.PgV <- glm(Outcome ~ Treatment * ProgVar + Age + Gender + Outcome_BL,
                  data = Trial_Data, family = "binomial")
    ...
    Outcomes[i,] <- data.frame(rejection.Risk = if_else((HDI[1] < 0 & 0 < HDI[2]), 
                               ...
                               SSVS_Converge = Converge.SVSS,
                               Risk_Converge = Converge.Risk,
                               Htrg_Converge = Converge.Htrg,
...
#write_csv(Outcomes, "~/Bayesian_RiskModel_TEH/Simulation_Study/Output/Binary_Outcomes/No_Heterogeneity-CatProgVar-Noise.csv")
```
...
for (i in 1:N) {
    ...
    beta.GeneratingOutcome <- case_when(
      ### No Heterogeneity and Categorical Prognostic Variable
      Heterogeneity.type == "No Heterogeneity" & 
      ProgVar.type == "Categorical" ~  c(0.00,    # Intercept
                                         1.00,    # Treatment
                                         0.75,    # Categorical 
                                         0.00,    # Treatment * Categorical Predictor
                                         -0.25,   # Gender
      ...
      ### Heterogeneity and Categorical Prognostic Variable
      Heterogeneity.type == "Heterogeneity" & 
      ProgVar.type == "Categorical" ~  c(-1.10,    # Intercept
                                         1.00,    # Treatment
                                         0.75,    # Categorical 
      ...
      ### No Heterogeneity and Continous Prognostic Variable
      Heterogeneity.type == "No Heterogeneity" & 
      ProgVar.type == "Continuous" ~   c(-1.25,    # Intercept
                                         0.50,    # Treatment
                                         0.25,    # Continuous 
                                         0.00,    # Treatment * Continuous Predictor
                                         -0.50,   # Gender
    ...
    Converge.SVSS <- if_else(all(gelman.diag(JAGS_samps.SSVS, multivariate = FALSE)$psrf[,2] < 1.1),
                             "Converged", "Not Converged")
    ...
    ### Skipping if no variables were to be included
    if (length(included) == 0) print(Converge.SVSS); next
    ...
    Converge.Risk <- if_else(all(gelman.diag(JAGS_samps.Risk, multivariate = FALSE)$psrf[,2] < 1.1),
                             "Converged", "Not Converged")
    ...
    Converge.Htrg <- if_else(all(gelman.diag(JAGS_samps.Heterogeneity, 
                                             multivariate = FALSE)$psrf[,2] < 1.1),
                             "Converged", "Not Converged")
    ...
    mod.Age <- glm(Outcome ~ Treatment * Age + Gender + ProgVar,
                  data = Trial_Data, family = "binomial")
    
    mod.Gen <- glm(Outcome ~ Treatment * Gender + Age + ProgVar,
                  data = Trial_Data, family = "binomial")
    
    mod.PgV <- glm(Outcome ~ Treatment * ProgVar + Age + Gender,
                  data = Trial_Data, family = "binomial")
    
    pval.Age <- summary(mod.Age)$coefficients[6,4]
    pval.Gen <- summary(mod.Gen)$coefficients[6,4]
    pval.PgV <- summary(mod.PgV)$coefficients[6,4]
...
sum(Trial_Data$Outcome) / length(Trial_Data$Outcome)

#write_csv(Outcomes, "~/Bayesian_RiskModel_TEH/Simulation_Study/Output/Binary_Outcomes/Heterogeneity-CatProgVar-Noise.csv")
```
