...
Noise.types <- c("No Noise", "Noise")

#for(l in 1:length(Noise.types)) {
  
Noise.type <- Noise.types[2]
  ...
  for (i in 1:N) {
    ...
    beta.GeneratingOutcome <- case_when(
      ...
      ProgVar.type == "Continuous" ~  c(0.25,     # Intercept
                                         1.00,    # Treatment
                                         0.25,    # Continuous 
                                         0.25,    # Treatment * Continuous Predictor
                                         1.00,    # Outcome_BL
                                         0.75,    # Gender
                                         0.25),  # Age
      TRUE ~ NA)
    ...
    HDI_Noise <- hdi(Samps.Heterogeneity[,3]) |> unname()
    }
